rule rel_to_abs:
    input:
        rds=expand(OUT_DIR+"sWGS_fitting/{{project}}_{{bin}}kb/absolute_POST_down_sampling/{{sample}}/relative_cn_rds/{{project}}_{{sample}}_{{bin}}kb_relSmoothedCN.rds"),
        meta=expand(OUT_DIR+"sWGS_fitting/{{project}}_{{bin}}kb/absolute_POST_down_sampling/{{sample}}/{{project}}_{{sample}}_fitted_QC.tsv")
    output:
